...
export class LocationService {
    ...
    async getLocations(): Promise<ReliefLocation[]> {
        ...
        // Map each aid point to a ReliefLocation and return the array
        if (!aidPoints || aidPoints.length === 0) {
            return []; // Return empty array if no locations found
        }
        return aidPoints.map(this.mapAidPointToReliefLocation);
    }
